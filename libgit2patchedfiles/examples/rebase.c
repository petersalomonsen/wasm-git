...
int lg2_rebase(git_repository *repo, int argc, char **argv)
{
	...
	if (parse_args(&opts, repo, argc, argv)) {
		...
		fprintf(stderr, "       %s [--abort|--continue|--ours|--theirs]]\n", argv[0]);
		fprintf(stderr, "           --abort: Cancels an existing rebase.\n");
		fprintf(stderr, "           --continue: Continues a paused rebase.\n");
		fprintf(stderr, "           --ours: Start a new rebase and resolve conflicts using ours.\n");
		fprintf(stderr, "           --theirs: Start a new rebase and resolve conflicts using theirs.\n");
		error = -1;
		goto cleanup;
	}
...
static int start_rebase(rebase_options *options, git_repository *repo)
{
	int error = 0;
	git_rebase *rebase = NULL;
	git_rebase_options rebase_options_raw = GIT_REBASE_OPTIONS_INIT;
	git_merge_options merge_options_raw = GIT_MERGE_OPTIONS_INIT;

	if (options->ours) {
		merge_options_raw.file_favor = GIT_MERGE_FILE_FAVOR_OURS;
		rebase_options_raw.merge_options = merge_options_raw;
	} else if (options->theirs)  {
		merge_options_raw.file_favor = GIT_MERGE_FILE_FAVOR_THEIRS;
		rebase_options_raw.merge_options = merge_options_raw;
	}
		...
		switch (operation->type) {
			case GIT_REBASE_OPERATION_EDIT:
				...
				if (error == GIT_EUNMERGED) {
					fprintf(stderr,
					"\nRebase conflicted. "
					"Resolve conflicts and add resolved files to index.\n"
...
static int parse_args(rebase_options *options, git_repository *repo, int argc, char **argv)
{
	...
	// Now parsing merge strategy
	i++;
	if (i < argc) {
		if (strcmp(argv[i], "--ours") == 0) {
			options->ours = 1;
		} else if (strcmp(argv[i], "--theirs") == 0) {
			options->theirs = 1;
		}
	}
