...
static void print_blob_content(const git_blob *blob);
...
int lg2_diff(git_repository *repo, int argc, char *argv[])
{
	...
	struct diff_options o = {
		...
		-1, -1, 0, 0, -1, GIT_DIFF_FORMAT_PATCH, NULL, NULL, "."
	};
    ...
    if (o.output_full_file_content != -1) {
        // print out full file content for each file
        ...
        for (size_t i = 0; i < num_deltas; i++) {
            ...
            if (delta->status == GIT_DELTA_ADDED) {
                printf("Added File: %s\n", delta->new_file.path);
                if (git_blob_lookup(&blob2, repo, &delta->new_file.id) < 0) {
                    continue;
                }
                printf("Content of file in commit %s:\n", o.treeish2);
                print_blob_content(blob2);
                git_blob_free(blob2);
            } else if (delta->status == GIT_DELTA_MODIFIED) {
                printf("Modified File: %s\n", delta->new_file.path);
                if (git_blob_lookup(&blob1, repo, &delta->old_file.id) < 0 ||
                    git_blob_lookup(&blob2, repo, &delta->new_file.id) < 0) {
                    continue;
                }
                printf("Content of file in commit %s:\n", o.treeish1);
                print_blob_content(blob1);
...
static void parse_opts(struct diff_options *o, int argc, char *argv[])
{
	...
	for (args.pos = 1; args.pos < argc; ++args.pos) {
        ...
        else if (!strcmp(a, "--full-file-content"))
			o->output_full_file_content = 0;
