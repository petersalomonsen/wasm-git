/**
 * Javascript functions for emscripten http transport with WASMFS/OPFS backend
 * 
 * This builds on the async transport (post-async.js) and adds OPFS filesystem support
 * through Emscripten's WASMFS. OPFS provides better performance and quota management
 * compared to IDBFS.
 * 
...
FS.chmod = function(path, mode, dontFollow) { 
    if ((mode & 0o100000) > 0) {
        // workaround for libgit2 calling chmod with only S_IFREG set (permissions 0000)
